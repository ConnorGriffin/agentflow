...
def test_two_store_record_corruption_between_resolve_and_reserve_is_unreadable_without_capacity(
        tmp_path):
    path = tmp_path / "coordinator.db"
    store = Store(
        path,
    ...
    record = Record(
        "stage-record-corrupt-race-646", "review", "codex", 1,
        repo="octo/app", model="sol", complexity="deep")
    stored, *_ = store.submit(record)
    selection = routing.select_route(
        "octo/app", "review", "codex", "sol", complexity="deep")
    ...
    with pytest.raises(RouteAdmissionRefused) as unreadable:
        store.consume_admitted_launch(
            stored.identity, stored.revision, selection.route_id,
            reserve=reserve, before_reserve=corrupt_record)
    assert unreadable.value.code == "unreadable"
    assert reservations == []
    other_store.close()
    store.close()
...
def test_two_store_lock_between_resolve_and_reserve_is_unreadable_without_capacity(
        monkeypatch, tmp_path):
    ...
    record = Record(
        "stage-lock-race-646", "review", "codex", 1,
        repo="octo/app", model="sol", complexity="deep")
    stored, *_ = store.submit(record)
    selection = routing.select_route(
        "octo/app", "review", "codex", "sol", complexity="deep")
    ...
    def hold_write_lock():
        other_store._conn.execute("BEGIN IMMEDIATE")
    ...
    try:
        with pytest.raises(RouteAdmissionRefused) as unreadable:
            store.consume_admitted_launch(
                stored.identity, stored.revision, selection.route_id,
                reserve=reserve, before_reserve=hold_write_lock)
        assert unreadable.value.code == "unreadable"
        assert reservations == []
        assert not store._conn.in_transaction
    finally:
        other_store._conn.execute("ROLLBACK")

    assert store.resolve_admitted_launch(
        stored.identity, stored.revision, selection.route_id).route_cell.digest == cell.digest
    other_store.close()
    store.close()
