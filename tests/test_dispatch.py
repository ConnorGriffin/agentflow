...
def test_coordinated_phase_submits_to_the_coordinator_and_skips_the_legacy_build(monkeypatch):
    ...
    monkeypatch.setattr(dispatch.coordinated_build, "resolve_phase",
                        lambda rollout, sessions, **k: Phase(COORDINATED))
    ...
    monkeypatch.setattr(dispatch.coordinated_build, "reconcile_and_project",
                        lambda coordinator, _log=None: projected.append(coordinator) or [])
...
def test_draining_phase_launches_no_new_build_of_either_kind(monkeypatch):
    ...
    monkeypatch.setattr(dispatch.coordinated_build, "resolve_phase",
                        lambda rollout, sessions, **k: Phase(DRAINING))
    reconciled = []
    coord = object()
    monkeypatch.setattr(dispatch.coordinated_build, "reconcile_and_project",
                        lambda coordinator, _log=None: reconciled.append(coordinator) or [])
