...
def test_request_to_finding_fix_and_merge_are_typed_and_joined(tmp_path):
    ...
    assert store._read(finding.failure_event_id).failure_class == "original_defect"
    assert store._read(finding.review_action_event_id).review_action == "fix_before_completion"
    assert store._read(merged.event.links[1].target_event_id).subject == "disposition/merge/review-1"
...
def test_review_action_is_closed_and_stably_names_axis_pass_and_sequence(tmp_path):
    ...
    assert all(store._read(first.review_action_event_id).subject
               != store._read(item.review_action_event_id).subject for item in changed)
...
def test_attack_objection_publicly_retains_a_stable_opaque_reference(tmp_path):
    ...
    assert store._read(first.event.event_id).subject == first.objection_id
    ...
    assert any(store._read(link.target_event_id).producer_kind == "objection"
...
def test_fix_lineage_is_public_for_ordinary_and_fix_introduced_paths(tmp_path):
    ...
    assert store._read(ordinary.event.event_id).revision == "e" * 40
    assert store._read(parent.target_event_id).revision == "c" * 40
    defect = producer.fix(FixFact("review-1", "c" * 40, "f" * 40, (first.finding_id,), request,
        "a" * 64, "reproduced", 3, "fix_introduced_defect"))
    failure = store._read(defect.failure_event_id)
...
def test_settlement_distinguishes_park_and_retains_attempt_join(tmp_path):
    ...
    disposition = store._read(parked.event.links[1].target_event_id)
