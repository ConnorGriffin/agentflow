...
2. Read the **park comment** for the review findings — the source-of-truth format is
   `gate.park`'s: a `- **<severity>** <file>:<line> — <summary>` line per finding. Take the
   **blocking** ones as your worklist; read any human replies since the park, too.
...
4. Clear each **blocking** finding (advisories are judgment calls) and keep the repo's test
   gate green. Same output contract as a coordinated Revise (`loop.REVISE_PROMPT`), driven by
   hand only because this command is explicitly taking over a parked human boundary.
...
- `revise` **never merges and never re-triggers the daemon** — a parked PR stays yours to
  merge (ADR 0017). It clears the blocking findings + conflicts the park comment named; it
  is not a fresh review, and it touches nothing in the `profile:` gate.
